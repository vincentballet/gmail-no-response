...
function processUnresponded() {
  ...
  threadMessages.forEach(function(messages, i) {
    var thread = threads[i],
        ...
        isgcal = lastMessage.getSubject().includes("(CEST)") || lastMessage.getSubject().includes("(CET)") || lastMessage.getSubject().includes("(RET)") || lastMessage.getSubject().includes("GMT");  // I don't want to hear about it when it's a gmeet invite

    if (!isgcal && isMe(lastFrom) && !isMe(lastTo) && lastMessageIsOld && !threadHasLabel(thread, ignoreLabel)) {
